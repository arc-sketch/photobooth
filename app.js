...
let overlayText = '', textFont = 'Pacifico', textColor = '#ffffff', textSize = 32;
...
document.querySelectorAll('.nav-tab').forEach(tab => {
    tab.addEventListener('click', () => {
        ...
        document.querySelectorAll('.nav-tab').forEach(t => t.classList.toggle('active', t.dataset.view === tab.dataset.view));
    });
});
...
function fxNeon(ctx,f,t,face){if(!face)return;const pts=face.landmarks.getJawOutline();['#ff6b9d','#c44dff','#6bc5ff'].forEach((c,i)=>{ctx.beginPath();ctx.strokeStyle=c;ctx.lineWidth=3;ctx.shadowColor=c;ctx.shadowBlur=15+Math.sin(t*0.005+i)*8;pts.forEach((p,j)=>j===0?ctx.moveTo(p.x,p.y):ctx.lineTo(p.x,p.y));ctx.stroke();});ctx.shadowBlur=0;}
...
document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', () => { document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); currentFilter = btn.dataset.filter; video.className = currentFilter !== 'none' ? 'filter-' + currentFilter : ''; });
});
...
document.querySelectorAll('.effect-btn').forEach(btn => {
    btn.addEventListener('click', () => { document.querySelectorAll('.effect-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); currentEffect = btn.dataset.effect; });
});
...
// PREVIEW
// ============================================================
function showSinglePreview() { previewOverlay.style.display = 'block'; $('preview-title').textContent = 'Nice shot!'; stickers = []; overlayText = ''; selectedSticker = null; currentTemplate = 'none'; document.querySelectorAll('.tpl-btn').forEach(b => b.classList.remove('active')); document.querySelector('.tpl-btn[data-template="none"]').classList.add('active'); $('photo-caption').value = ''; $('overlay-text').value = ''; document.querySelectorAll('.sticker-btn').forEach(b => b.classList.remove('active')); renderResult(); }
function showMultiPosePreview() { previewOverlay.style.display = 'block'; $('preview-title').textContent = 'Your 4-pose strip!'; stickers = []; overlayText = ''; currentTemplate = 'strip4'; document.querySelectorAll('.tpl-btn').forEach(b => b.classList.remove('active')); document.querySelector('.tpl-btn[data-template="strip4"]').classList.add('active'); $('photo-caption').value = ''; $('overlay-text').value = ''; renderResult(); }

document.querySelectorAll('.tpl-btn').forEach(btn => { btn.addEventListener('click', () => { document.querySelectorAll('.tpl-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); currentTemplate = btn.dataset.template; renderResult(); }); });
...
function switchView(name) { document.querySelectorAll('#screen-booth .view').forEach(v => v.classList.remove('active')); $('view-' + name).classList.add('active'); document.querySelectorAll('.nav-tab').forEach(t => t.classList.toggle('active', t.dataset.view === name)); }

// ============================================================
// STICKERS
// ============================================================
document.querySelectorAll('.sticker-btn').forEach(btn => { btn.addEventListener('click', () => { document.querySelectorAll('.sticker-btn').forEach(b => b.classList.remove('active')); if (selectedSticker === btn.dataset.sticker) selectedSticker = null; else { btn.classList.add('active'); selectedSticker = btn.dataset.sticker; } }); });
...
function renderPhotoStrip(w,h){if(multiPoses.length>=4){const g=8,p=16;resultCanvas.width=w+p*2;resultCanvas.height=h*4+g*3+p*2;resultCtx.fillStyle='#1a1a2e';resultCtx.fillRect(0,0,resultCanvas.width,resultCanvas.height);for(let i=0;i<4;i++)resultCtx.drawImage(multiPoses[i],p,p+i*(h+g),w,h);resultCtx.fillStyle='rgba(255,107,157,0.1)';resultCtx.fillRect(0,0,resultCanvas.width,resultCanvas.height);}else renderNone(w,h);}
function renderPolaroid(w,h){const p=40,b=80;resultCanvas.width=w+p*2;resultCanvas.height=h+p+b;resultCtx.fillStyle='#fff';resultCtx.fillRect(0,0,resultCanvas.width,resultCanvas.height);resultCtx.drawImage(multiPoses[0],p,p,w,h);resultCtx.fillStyle='#333';resultCtx.font='20px Pacifico,cursive';resultCtx.textAlign='center';resultCtx.fillText('\u2764 Our Moment \u2764',resultCanvas.width/2,h+p+50);}
...
$('btn-send-photo').addEventListener('click', () => {
    ...
    currentTemplate = 'none'; document.querySelectorAll('.tpl-btn').forEach(b => b.classList.remove('active'));
    document.querySelector('.tpl-btn[data-template="none"]').classList.add('active');
